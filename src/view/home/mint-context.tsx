...
const defaultValuesContext = {
  ...
  imageBlob: [],
  isLoading: false,
  resultSubmit: { CID: null, contractAddress: "", tx: null, tokenID: null },
};
...
export const MintNftContextProvider = (props: {
  ...
  const validateStateInput = (): boolean => {
    // console.log("🚀 ~ file: mint-context.tsx:85 ~ validateStateInput ~ state:", state);
  ...
  useEffect(() => {
    if (contractState.status === "Success") {
      ...
      if (contractState.receipt) {
        ...
        const tokenId = Web3.utils.hexToNumber(contractState.receipt.logs[0].topics[3]);
        setResultSubmit({ ...resultSubmit, tx: contractState.receipt.transactionHash, tokenID: tokenId });
      }
  ...
  const doSubmitNft = async (): Promise<void> => {
    ...
    if (account && library) {
      ...
      setResultSubmit({
        CID: resultImageUpload.IpfsHash,
